...
import rtv_api
# Speed & stride telemetry deprecated - using Master Handicapping System
...
# [scan_speed_and_stride removed - speed and stride deprecated]
...
# Master System (CD Winner + Placed LTO + Weight Advantage)
SYSTEM_DISPLAY = {"Tips": "Master System"}
    ...
    if df is None or df.empty:
        ...
        if verdict:
            st.caption(
                ...
                "all**, so it can top a big-priced runner; and the **tips list** (⚡⭐🔔) is the Master System "
...
# ==============================================================================
elif st.session_state["nav_view"] == "🎯 Master System":
    _tips_tabs = st.tabs([
        "🎯 Master Qualifiers (Our System)",
    ...
    with _tips_tabs[0]:
        try:
            ...
            st.error(f"Master System could not load: {_our_exc}")
            ...
            for c_name, r in races_to_show:
                ...
                with st.expander(f"🏇 {r_time} {c_name} — {r_title}", expanded=(sel_ai_meeting != "All Meetings Today")):
                    try:
                        ...
                        st.warning(f"Could not load details for {r_time} {c_name}: {_ai_e}")
...
# ==============================================================================
    ...
    tab_tips, tab_ai, tab_ew, tab_all, tab_daily = st.tabs([
        "🎯 Master System",
        "🤖 AI System",
        "💱 Exchange EW Edge",
    ...
    # Tab 1: Master Handicapping System
    with tab_tips:
        st.subheader("🎯 Master Handicapping System (Our System)")
        st.caption(
            "Empirically proven system: **Course & Distance Winner**, **Peak Recent Form (Placed 2nd or 3rd LTO)**, "
            "**Market Support**, and **Weight Advantage** (Same Mark: 35.1% SR, +20.3% ROI / Weight Relief: 37.0% SR, +38.9% ROI)."
        )
        tips_data = res_df[res_df["system_name"] == "Tips"]
        render_system_metrics_and_table("Master System", tips_data)
        ...
        if daily_src.empty:
            ...
            if settled.empty:
                ...
                if not tips_only.empty:
                    st.markdown("**Master System by category, per day** - which angle is actually paying")
                    ...
                    st.markdown("**Master System by angle — strike rate and ROI** "
    ...
    with tab_all:
        ...
        st.markdown(f"**System leaderboard** "
                    f"({'all logged dates' if chosen_date == 'ALL' else chosen_date}) — "
                    "one row per system, comparing Master System, AI System, and Exchange EW Edge")
