# Racing Form System - price & market tooling
...
One exception is now under test: the **stride systems** in `STRIDE_SYSTEM.md`
(previous-run top speed, and previous-run stride length). They survive real BSP
prices *and* commission on a rebuilt, complete-field sample — but they have not
been forward-tested, so they are a candidate, not a system.
...
| `betfair_prices.bat` | Betfair login check, exchange price snapshot, book-vs-exchange comparison | Any time |
| `watch_prices.bat [mins]` | Snapshots every 60s (default 30 min) to measure how fast the accounts reprice | Before the off |
| `finish_bsp.bat YYYY-MM-DD` | Waits for Betfair's SP file, imports it, rebuilds the log, prints the report | The morning after racing |
...
| `finish_bsp.bat YYYY-MM-DD` | Waits for Betfair's SP file (they ship a day's prices the NEXT day), imports it, rebuilds the price log, prints the book-vs-SP report | The morning after racing |
...
| **Stride speed system, net of 2% commission at BSP** | **+9.46% over 38,420 bets** (candidate, forward test pending — `STRIDE_SYSTEM.md`) |
...
- The stride systems (**SPEED** = previous-run top speed, **STRIDE** = previous-run
  stride length) are the first selection rules to survive real BSP prices *and*
  commission. They are documented separately, old numbers and new, in
  `STRIDE_SYSTEM.md` — the earlier +12.89% figure came from a frame missing ~7%
  of every field and must not be quoted.
...
- `STRIDE_SYSTEM.md` - the SPEED and STRIDE systems documented separately: the
  old (+12.89%) numbers that came from a broken frame, the rebuilt clean-frame
  results (+9.46% / +6.47% net of commission), the look-ahead audit, and the
  price rule (BSP only).
- `DATA_AUDIT.md` - the three databases, the missing-column bug that stopped
  the live scraper, the weight rebuild, and what was removed.
- `SYSTEM_NOTES.md` - database schemas and column names.
- `FRESH_NOTE.md` - the "no dashboard, no generated JSON" standard.

Historical, superseded documents are in `archive/audit_2026-09-15/stale_docs/`:
the look-ahead/BSP-margin findings and the original Master Lay specification.
The lay and back-to-lay systems they describe were invalidated at real prices
(see `ODDS_API_FINDING.md`).
