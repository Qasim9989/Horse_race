...
def main() -> None:
    ...
    for s in (0.0, 0.02, 0.04, 0.06, 0.08, 0.10, 0.15):
        pl = (top["MorningWAP"] * (1 - s) / top["BSP_TRUE"] - 1.0) * (1 - COMMISSION) * 2
        print(f"  back price {s * 100:>4.0f}% worse   mean GBP{pl.mean():>+6.2f} a trade   "
    ...
    for i in np.argsort(imp.importances_mean)[::-1][:8]:
        print(f"  {FEATS[i]:<22} {imp.importances_mean[i]:+.4f}")
