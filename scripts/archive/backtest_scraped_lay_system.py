...
def run_backtest():
    print("="*95)
    print("  ZERO-LOOKAHEAD LAY SYSTEM BACKTEST ON SCRAPED RACINGTV DATA (2023 - 2026)")
    print("="*95)

    conn = pyodbc.connect(CONN_STR)
    
    print("\n[1/4] Loading all runner results & RaceIQ metrics from RACINGTV_2023_2026...")
    sql = """
    SELECT 
        r.RaceDate,
        r.RaceTime,
    ...
    print(f"  Loaded {len(df):,} total runner rows.")
    ...
    print("\n[2/4] Computing Point-in-Time Last-Time-Out (LTO) Features...")
    
    # Sort strictly by horse and date for point-in-time LTO calculation
    ...
    print("\n[3/4] Filtering Handicap Races and Identifying Lay Targets...")
    
    # Handicap races with field size >= 5
    ...
    # Identify worst-ranked runners per race
    df_valid['RankWorst'] = df_valid.groupby(['RaceDate', 'CourseName', 'RaceTime'])['Score'].rank(method='min', ascending=True)

    # Lay Condition: Worst score in race (RankWorst <= 2), negative score (Score < 0), and Odds <= 20.0
    ...
    # Calculate Lay P&L (£1 Liability / Flat Stake Simulation)
    # If Loss (Winner): - (DecSP - 1)
    # If Win for Lay (Loser): +0.95 (5% commission deducted)
    qualifiers['LayPNL'] = np.where(qualifiers['IsWin'], -(qualifiers['DecSP'] - 1.0), 0.95)

    qualifiers['YearMonth'] = qualifiers['RaceDate'].dt.strftime('%Y-%m')

    print(f"  Found {len(qualifiers):,} total Lay Qualifiers across all races.")

    print("\n[4/4] Monthly Performance & Drawdown Analysis...")
    ...
    print("="*95)
    print("  MONTH-BY-MONTH LAY SYSTEM PERFORMANCE (2023 - 2026)")
    print("="*95)
    ...
    print(f"  - Total Net P&L:        +£{total_pnl:,.2f} (@ £1 flat stake / 5% comm)")
